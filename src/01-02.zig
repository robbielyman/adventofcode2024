...
fn process(allocator: std.mem.Allocator, input: []const u8) !i32 {
    ...
    var right: std.AutoHashMapUnmanaged(i32, i32) = .{};
    ...
    while (iterator.next()) |line| {
        ...
        const res = try right.getOrPut(allocator, right_in);
    ...
    var output: i32 = 0;
    for (left.items) |l| {
        const r = right.get(l) orelse continue;
        output += l * r;
    }
    return output;
}
...
test {
    ...
    try std.testing.expectEqual(31, output);
}
